...
async def setup_registry_data(registry_instance: AgentRegistry):
    ...
    await registry_instance.register(
        AgentRegistration(
            agent_id="data-agent",
            ...
            skills=[
                ...
                Skill(
                    name="data_cleaning", description="Data preprocessing and cleaning"
                ),
            ],
    ...
    await registry_instance.register(
        AgentRegistration(
            ...
            skills=[
                Skill(
                    name="research",
                    description="Research skills for finding information",
                ),
                Skill(
                    name="summarization", description="Summarize information concisely"
                ),
            ],
    ...
    async def test_capability_update(self, registry_instance: AgentRegistry):
        ...
        for agent, score in results:
            if agent.agent_id == agent_id_to_update:
                ...
                ), "New capability 'translate_text' not found."
                # Check if old capability 'summarize_text' was removed (as it wasn't in the update)
                assert not any(
                    cap.name == "summarize_text" for cap in agent.capabilities
                ), "Old capability 'summarize_text' should have been removed."
                # Check tags
