...
AgentConnect is a lightweight framework for building autonomous, interoperable agents that can:

- Build and run as independent peers (no central controller)
- Be discovered by rich, searchable profiles (capabilities, skills, metadata)
- Communicate securely through a hub using signed messages
...
.. admonition:: Mental model: Describe → Discover → Talk
   ...
   - Describe: Define an agent's profile (identity, capabilities, skills, tags) → :doc:`core/agent_profile_and_capabilities`
   - Discover: Find agents by capability or semantics → :doc:`core/discovery/index`
   - Talk: Route signed messages via the hub → :doc:`core/communication/index`
...
3. **Identity Verification**: Performs identity verification during registration
...
Now that you understand the core concepts of AgentConnect, explore:

- :doc:`core/first_agent` - Build and run your first local agents
- :doc:`core/agent_profile_and_capabilities` - Model agents for better discovery
- :doc:`core/discovery/index` - Search agents by capability and semantics
- :doc:`core/communication/index` - Perform Agent-to-Agent communication with the hub
