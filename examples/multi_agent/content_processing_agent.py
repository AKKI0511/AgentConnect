...
def create_content_processing_agent(
    ...
    content_processing_skills = [
        ...
        Skill(
            name="text_chunking", description="Split large texts into manageable chunks"
        ),
        Skill(
    ...
    def html_to_markdown(html_content: str) -> Dict[str, str]:
        ...
        try:
            ...
            return {"markdown": converted_docs[0].page_content, "success": True}
        except Exception as e:
        ...
        try:
            ...
            if source.startswith(("http://", "https://")):
                ...
                if not temp_path:
                    ...
                    return {
                        "error": f"File not found. Checked these paths: {', '.join(possible_paths)}"
                    }
    ...
    def load_pdf(
        pdf_source: Union[PDFSourceInput, str, Dict[str, str]],
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if isinstance(pdf_source, str):
                ...
                # Unknown input type
                return {
                    "error": f"Unsupported input type: {type(pdf_source)}. Expected string, dictionary, or PDFSourceInput."
                }
            ...
            if uuid_pattern.search(source_str) or (
                "downloads" in source_str and source_str.endswith(".pdf")
            ):
                ...
                for pdf_file in pdf_files:
                    if basename in pdf_file or (
                        uuid_pattern.search(pdf_file)
                        and uuid_pattern.search(source_str)
                    ):
            ...
            if source_str.startswith("downloads"):
                ...
                if not os.path.exists(source_str):
                    potential_path = os.path.join(
                        downloads_dir, os.path.basename(source_str)
                    )
    ...
    content_processing_agent = AIAgent(
        ...
        external_callbacks=[
            ToolTracerCallbackHandler(
                agent_id="content_processing_agent", print_tool_activity=True
            )
        ],
    )
