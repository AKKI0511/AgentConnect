...
├── service.py                      # Registry API server
...
Registry process settings live in `agentconnect.config.servers`.
...
Developer note (for contributors): You can also run via uvicorn or module path: `uvicorn agentconnect.index.service:app` or `python -m agentconnect.index.service`.
...
```powershell
Copy-Item 'agentconnect/index/.env.example' '.env'
```
...
- Docker Compose: reference the file with `env_file: .env` (copy `agentconnect/index/.env.example` â†’ `.env` first). See `agentconnect/index/docker-compose.example.yml` for a complete minimal example.
...
- Kubernetes: create a ConfigMap from the `.env` and a Secret for sensitive keys, then load both via `envFrom` (do not commit secrets to VCS):
  ...
  Deployment snippet (see `agentconnect/index/k8s/deployment.example.yaml`):
...
- **Client Libraries**: See `agentconnect/index/` for programmatic access
