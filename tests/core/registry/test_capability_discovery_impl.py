...
class TestEmbeddingUtils:
    ...
    def test_calculate_similarity(self):
        ...
        test_cases = [
            ...
            (
                ...
                "weather prediction",
                0.33,
            ),  # Some overlap - actual calculation is ~0.33
        ...
        for text1, text2, expected in test_cases:
            ...
            # Print the result
            print(
                f"  {text1:<20} | {text2:<20} | {Colors.YELLOW}{similarity:.2f}{Colors.ENDC}"
            )
    ...
    def test_cosine_similarity(self):
        ...
        for vec1, vec2, expected in test_cases:
            ...
            # Print the result
            print(
                f"  {str(vec1):<15} | {str(vec2):<15} | {Colors.YELLOW}{similarity:.4f}{Colors.ENDC}"
            )
    ...
    def test_check_semantic_search_requirements(self):
        ...
        print_success(
            "Completed semantic search requirements check. Please verify results above."
        )
    ...
    def test_create_huggingface_embeddings(self):
        ...
        if embeddings_model:
            ...
            print_success(
                "Completed embedding generation and validation. Please verify results above."
            )
        else:
            print_warning(
                "Embeddings model creation failed, skipping embedding generation test"
            )
...
class TestQdrantClient:
    ...
    async def test_initialize_qdrant_clients(self):
        ...
        try:
            ...
            if sync_client and async_client:
                ...
                print_step(
                    f"Testing sync client - creating collection {collection_name}"
                )
                ...
                print_step(
                    f"Testing async client - checking collection {collection_name}"
                )
                exists = await async_client.collection_exists(collection_name)
                assert exists, f"Collection {collection_name} should exist"
                print_success(
                    f"Async client confirmed collection {collection_name} exists. Please verify."
                )
    ...
    @pytest.mark.skipif(
        not check_semantic_search_requirements()["qdrant"]
        or not check_semantic_search_requirements()["embedding_model"],
    ...
    async def test_init_qdrant_collection(self):
        ...
        print_step("Initializing in-memory Qdrant clients")
        qdrant_config = VectorSearchSettings.model_validate(
            {
                "deployment": {"type": "in_memory"},
...
class TestSearch:
    ...
    async def test_fallback_string_search(self):
        ...
        results = await fallback_string_search(
            "weather forecast", test_registrations, limit=10, similarity_threshold=0.1
        )
    ...
    async def test_find_by_capability_name(self):
        ...
        print_success(
            "Completed exact capability matches. Please verify results above."
        )
        ...
        print_success(
            "Completed semantic search fallback. Please verify results above."
        )
...
class TestIndexing:
    ...
    async def test_extract_capability_index(self):
        ...
        print_success(
            "Completed capability index extraction. Please verify results above."
        )
    ...
    async def test_update_capability_embeddings(self):
        ...
        collection_created = await init_qdrant_collection(
            async_client, embeddings_model, collection_name, {"use_quantization": False}
        )
        ...
        print(
            f"  Collection contains {Colors.YELLOW}{count_result.count}{Colors.ENDC} points"
        )
