...
`find`, `ask`, `tell`, `get_result`, and `get_history`. Slow work returns a Ticket. Keep `ticket.id` for `get_result` and `thread_id` for `get_history`. The roster is the resource `agentconnect://team/roster`. Advertised tool argument schemas are the public argument types, including omit-only optional fields and bounds. Extra Team tools keep the schema their callable declared.
