...
def extract_capabilities_index(
...
class TestCapabilityDiscoveryE2E:
    ...
    async def test_capability_discovery_string_matching(
        ...
        test_registrations: Dict[str, AgentRegistration],
        ...
        print_success(
            "Completed search for all weather forecast agents. Please verify results above."
        )
        ...
        print_success(
            "Completed search for stock price agent. Please verify results above."
        )
        ...
        print_success(
            "Completed search for non-existent capability. Please verify results above."
        )
    ...
    async def test_semantic_search_with_embeddings(
        ...
        print_success(
            "Completed search for agents that can predict weather conditions. Please verify results above."
        )

        print_step(
            "Searching for agents that can 'generate images based on text descriptions'"
        )
        results = await discovery_service.find_by_capability_semantic(
        ...
        print_success(
            "Completed search for agents that can generate images. Please verify results above."
        )
        ...
        print_success(
            "Completed search with organization filter. Please verify results above."
        )

    @pytest.mark.asyncio
    async def test_capability_embedding_updates(
        self,
        discovery_service: CapabilityDiscoveryService,
        test_registrations: Dict[str, AgentRegistration],
    ):
        ...
        print_success(
            "Completed search for agent with updated capabilities. Please verify results above."
        )
    ...
    @pytest.mark.asyncio
    async def test_fallback_when_embeddings_unavailable(
        self,
        test_registrations: Dict[str, AgentRegistration],
        capabilities_index: Dict[str, Set[str]],
    ):
        ...
        assert (
            len(results) >= 0
        )  # May return results if string similarity is good enough
