...
- Bounded work and retention. `send` and `reply` share `max_message_bytes`. HTTP JSON bodies use the same cap from `Content-Length` before the rest of the body is read. Join reports `max_deadline_seconds`, `max_open_tickets`, `replay_horizon_seconds`, and `max_retained_bytes`. Duplicate protection lasts for the documented replay window. Reuse of a Message id after that window fails with `id_conflict` while any Delivery, Ticket, history entry, or replay record still names it. Thread trim does not drop queued or leased work. Releasing the last owner of an unthreaded event deletes its Message body and retained-byte charge. Uncaught handler exceptions store `The handler failed.`
