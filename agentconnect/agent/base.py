...
class BaseAgent(ABC):
    ...
    async def process_message(self, message: Message) -> Optional[Message]:
        ...
        if (
            ...
            # Determine the appropriate message type based on the request type
            kind = MessageKind.RESPONSE if is_collaboration_request else CONTROL_STOP
        ...
        if message.control == CONTROL_STOP or "__EXIT__" in message.content:
            self.end_conversation(message.sender_id)
    ...
    async def _process_message_and_respond(self, message: Message):
        ...
        try:
            ...
            response_sent = False
            if response and response.control != CONTROL_IGNORE:
            ...
            response_type = response.kind.value if response else None
            logger.debug(
