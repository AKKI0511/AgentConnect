...
def run_tests(args):
    ...
    impl_test_file = os.path.join(
        script_dir, "core", "registry", "test_capability_discovery_impl.py"
    )
    e2e_test_file = os.path.join(
        script_dir, "core", "registry", "test_capability_discovery_e2e.py"
    )
    integration_test_file = os.path.join(
        script_dir, "core", "registry", "test_integration.py"
    )

    # Check if the test files exist
    if not os.path.exists(impl_test_file):
        print(
            f"{Colors.RED}Error: Implementation test file not found at {impl_test_file}{Colors.ENDC}"
        )
        return 1

    if not os.path.exists(e2e_test_file):
        print(
            f"{Colors.RED}Error: End-to-end test file not found at {e2e_test_file}{Colors.ENDC}"
        )
        return 1

    if not os.path.exists(integration_test_file):
        print(
            f"{Colors.RED}Error: Integration test file not found at {integration_test_file}{Colors.ENDC}"
        )
        return 1
...
def main():
    ...
    parser.add_argument(
        "--impl", action="store_true", help="Run implementation tests only"
    )
    ...
    parser.add_argument(
        "--quiet", action="store_true", help="Suppress test output except for errors"
    )
